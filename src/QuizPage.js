...
const QuizPage = () => {
  ...
  const fetchQuizList = async () => {
    try {
      const response = await axios.get("http://localhost:9192/api/quizzes");
  ...
  const fetchQuizQuest = async (quizId) => {
    try {
      const response = await axios.get(
        `http://localhost:9192/api/quizzes/getQuizQuestById/${quizId}`
      );
    ...
    try {
      ...
      for (const userAnswer of selectedOptions) {
        ...
        const response = await axios.post(
          "http://localhost:9192/api/user-answers",
          userAnswers
        );
